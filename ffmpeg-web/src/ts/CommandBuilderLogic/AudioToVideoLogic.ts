    ...
    */
    ...
    for (const singleFile of pickedFiles) {
        ...
        try {
            ...
            ffmpegOperation.operationComplete();
            if (chosenConversionOptions.restorePTS) { // This *might* help fixing wrong timestamps in the output file by changing the presentation timestamps. I actually am not sure if it does something, or if it worked for unrelated reasons. In any case, it's disabled by default.
                const videoFilterPosition = hardwareAcceleration.after.indexOf("-vf"); // Look if the hw acceleration provides a video filter. In this case (won't be -1), add it to the filter argument. Otherwise, we'll push the filter
