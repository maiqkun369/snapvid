  ...
  if (tasks.length === 0) {
    return (
      <div className="text-center py-10">
        <p className="text-white/20 text-sm font-light">No downloads yet</p>
      </div>
    );
  }
            ...
            >
              ...
              <div className="flex items-center gap-2">
                {task.status === 'completed' && (
                  <button
                    onClick={() => handleDownloadFile(task.id, task.filename || `${task.title}.mp4`)}
                    className="p-2 text-emerald-400 hover:text-emerald-300 hover:bg-white/[0.05] rounded-lg transition-colors"
                    title="下载文件"
                  >
